...
def get_generator():
    model_g = FPNInception(norm_layer=get_norm_layer('instance'))
    return nn.DataParallel(model_g)


def get_model() :
    model = get_generator()
    model.load_state_dict(torch.load("/Users/meiannn/Documents/GitHub/DeblurGanv2/fpn_inception.h5", map_location=torch.device('cpu'))['model'])
    model.eval()
    return model
